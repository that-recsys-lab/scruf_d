...
class WhalrusWrapperMechanism(ChoiceMechanism):
    ...
    def setup(self, input_props, names=None):
        super().setup(
            input_props,
            names=self.configure_names(WhalrusWrapperMechanism._PROPERTY_NAMES, names),
        )
    ...
    def wrap_ballots(self, bcoll):
        ...
        for ballot in bcoll.get_ballots():
            ballot_dict = {
                entry.item: entry.score for entry in ballot.prefs.get_results()
            }
        ...
        if weights is None:
            if self.tiebreak_class is None:
                ...
                self.whalrus_rule = self.whalrus_class(
                    ballots, tie_break=self.tiebreak_class
                )
        else:
            if self.tiebreak_class is None:
...
mechanism_specs = [
    ("whalrus_scoring", WhalrusWrapperScoring),
