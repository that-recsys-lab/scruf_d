...
class Scruf:
        ...
        def __init__(self, config):
            if config is not None:
                ...
                self.rand = random.Random(
                    get_value_from_keys(
                        ["parameters", "random_seed"], config, default=420
                    )
                )
                ...
                # Mechanisms
                amech_class = get_value_from_keys(
                    ["allocation", "allocation_class"], config
                )
                amech = AllocationMechanismFactory.create_allocation_mechanism(
                    amech_class
                )
                ...
                # Parameters
                self.output_list_size: int = get_value_from_keys(
                    ["parameters", "list_size"], config
                )
                self.iterations: int = get_value_from_keys(
                    ["parameters", "iterations"], config
                )
    ...
    def setup_experiment():
        ...
        # Mechanisms
        amech_props = Scruf.get_value_from_keys(
            ["allocation", "properties"], default={}
        )
    ...
    def run_loop(self, iterations=-1, restart=True, progress=False):
        ...
        if progress:
            user_data = tqdm(
                Scruf.state.user_data.user_iterator(iterations, restart=restart)
            )
        else:
